...
class AppDebugger {
  ...
  Future<void> _testEventLoading(BuildContext context) async {
    ...
    try {
      ...
      _testResults['featured_events'] = eventsProvider.featuredEvents.isNotEmpty;
  ...
  Future<void> _testSearch(BuildContext context) async {
    ...
    try {
      ...
      // Test filters
      eventsProvider.applyFilters(
        category: 'music',
        maxDistance: 50,
        priceFilter: 'low',
      );
      
      _testResults['filters'] = eventsProvider.filteredEvents.isNotEmpty;
  ...
  Future<void> _testFavorites(BuildContext context) async {
    ...
    try {
      ...
      if (eventsProvider.events.isNotEmpty) {
        ...
        // Toggle favorite
        await eventsProvider.toggleFavorite(testEvent.id);
        _testResults['toggle_favorite'] = true;
        
        // Get saved events
        final savedEvents = eventsProvider.savedEvents;
        _testResults['saved_events'] = savedEvents.isNotEmpty;
      }
  ...
  Future<void> _testChat(BuildContext context) async {
    ...
    try {
      ...
      if (authProvider.currentUser != null) {
        // Create chat session
        final session = await chatProvider.createNewSession(
          userId: authProvider.currentUser!.id,
          type: ChatType.eventDiscovery,
          title: 'Test Chat',
        );
        
        _testResults['chat_session'] = session != null;
      ...
      debugPrint('✅ Chat test completed');
    } catch (e) {
  ...
  Future<void> _testLocation() async {
    ...
    try {
      ...
      _testResults['location_permission'] = hasPermission;
      ...
      if (hasPermission) {
        ...
        _warnings.add('Location permission not granted');
      }
  ...
  Future<void> _testAPIConnections() async {
    ...
    try {
      ...
      try {
        await rapidAPIService.testConnection();
  ...
  Future<void> _testMemoryLeaks() async {
    ...
    try {
      ...
      if (currentSizeBytes > 100 * 1024 * 1024) { // > 100MB
        _warnings.add('Image cache is large: ${currentSizeBytes ~/ (1024 * 1024)}MB');
      }
      ...
      SchedulerBinding.instance.addTimingsCallback((timings) {
        for (final timing in timings) {
          if (timing.rasterDuration.inMilliseconds > 16) {
            _warnings.add('Slow frame: ${timing.rasterDuration.inMilliseconds}ms');
          }
        }
      });
