          ...
          onScaleUpdate: (details) {
            setState(() {
              ...
              if (details.scale != 1.0) {
                _zoomLevel = math.max(
                  0.5,
                  math.min(3.0, _zoomLevel * details.scale),
                );
              }
    ...
    return AnimatedPositioned(
      ...
      child:
          GestureDetector(
              ...
              .animate(
                delay: Duration(
                  milliseconds: widget.events.indexOf(event) * 100,
                ),
              )
              .scale(
...
class DemoMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 8; i++) {
      canvas.drawRect(
        Rect.fromLTWH(size.width * 0.45 + (i * 8), size.height * 0.52, 4, 12),
        landmarkPaint,
      );
    }
  ...
  void _drawScale(Canvas canvas, Size size) {
    ...
    canvas.drawLine(endPoint, Offset(endPoint.dx, endPoint.dy - 5), scalePaint);
    ...
    textPainter.paint(
      canvas,
      Offset(startPoint.dx + scaleLength / 2 - 8, startPoint.dy + 5),
    );
  }
