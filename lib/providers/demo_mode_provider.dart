...
class DemoModeProvider extends ChangeNotifier {
  ...
  Map<String, List<EventInteractionData>> _eventInteractions = {};
  ...
  void _generateEventInteractions() {
    ...
    for (int i = 1; i <= 7; i++) {
      ...
      final numInteractions = _random.nextInt(5) + 2; // 2-6 interactions per event
  ...
  void _simulateNewNotification() {
    ...
    final newNotification = DemoNotification(
      ...
      title: 'SomethingToDo',
      message: notificationMessages[_random.nextInt(notificationMessages.length)],
      timestamp: DateTime.now(),
      isRead: false,
  ...
  void _simulateEventInteraction() {
    ...
    if (_eventInteractions[eventId]!.length > 10) {
      _eventInteractions[eventId] = _eventInteractions[eventId]!.take(10).toList();
    }
