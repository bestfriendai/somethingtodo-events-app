  ...
  void _handleShakeDetected(BuildContext context) {
    ...
    if (_shakeCount == 1) {
      ...
      DelightService.instance.showConfetti(
        context,
        customMessage:
            'Shake it like a Polaroid picture! You are the shake champion! 📸',
      );
    } else {
      final messages = [
      ...
      case 'like':
        if (count == 10) {
          ...
          _showAchievement(
            context,
            'Love Machine',
            'You have liked 50 events! Spread the love!',
          );
        }
        break;
      case 'share':
        if (count == 5) {
          _showAchievement(
            context,
            'Social Butterfly',
            'You have shared 5 events!',
          );
        } else if (count == 25) {
          _showAchievement(
            context,
            'Viral Sensation',
            'You have shared 25 events! You are making the world more fun!',
          );
        }
        break;
      case 'search':
        if (count == 20) {
          _showAchievement(
            context,
            'Explorer',
            'You have searched 20 times! You know what you want!',
          );
        }
        break;
      case 'refresh':
        if (count == 15) {
          _showAchievement(
            context,
            'Refresh Master',
            'You have refreshed 15 times! Persistence pays off!',
          );
        }
        break;
    }
...
class SoundEffectService {
  ...
  static SoundEffectService get instance =>
      _instance ??= SoundEffectService._();
