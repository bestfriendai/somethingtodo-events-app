...
class LoggingService {
  ...
  static void circuitBreakerOpen({String? tag}) {
    warning(
      '🚫 Circuit breaker opened - service temporarily disabled',
      tag: tag ?? 'CircuitBreaker',
    );
  }
  ...
  static void retryAttempt(
    ...
    debug(
      '🔄 Retry $attempt/$maxRetries for $operation in ${delayMs}ms',
      tag: tag ?? 'Retry',
    );
  }
  ...
  static void eventRetrievalStart(int targetEvents, {String? tag}) {
    info(
      '🔍 Starting enhanced event retrieval (target: $targetEvents events)',
      tag: tag ?? 'Events',
    );
  }
  ...
  static void parallelRequestsStart(int requestCount, {String? tag}) {
    info(
      '🚀 Executing $requestCount parallel requests...',
      tag: tag ?? 'Events',
    );
  }

  /// Log event retrieval completion
  static void eventRetrievalComplete(
    int eventCount,
    int sourceCount, {
    String? tag,
  }) {
    info(
      '✅ Retrieved $eventCount unique events from $sourceCount sources',
      tag: tag ?? 'Events',
    );
  }

  /// Log search failures
  static void searchFailed(
    String searchType,
    String details, {
    String? tag,
    Object? error,
    ...
    warning(
      '⚠️ $searchType search failed: $details',
      tag: tag ?? 'Search',
      error: error,
    );
  }
  ...
  static void timeout(String operation, {String? tag}) {
    warning(
      '⏰ $operation timed out, using partial results',
      tag: tag ?? 'Timeout',
    );
  }
}
